...
SOURCES = [{"title": "BassFox ", "fanart": "https://dl.dropbox.com/s/038p1oo392w3mor/fanart.jpg", "genre": "Tv Live", "date": "08.11.2015", "credits": "BassFox", "thumbnail": "http://i2.minus.com/j5HeempSScdjg.png", "url": "https://dl.dropbox.com/s/dgyqvy9w6zujtrk/bassfox.index.1.5.4.BSFXJLinkByBassfox-freeTV.BSFXprivate2015silocopiaslovolvereactulizarusalarazon.xml"}]
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR cyan][B]- FAVORITES CHANNEL BASSFOX [/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
#            ln+='</item>'
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(BassFox,Parental Block Desactivado,5000,"+icon+")")
        else:
    ...
    xbmc.executebuiltin("XBMC.Notification(BassFox,Parental block Activado,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
